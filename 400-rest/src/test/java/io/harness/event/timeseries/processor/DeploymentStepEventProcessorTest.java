...
public class DeploymentStepEventProcessorTest extends WingsBaseTest {
  @Mock TimeScaleDBService timeScaleService;
  @Inject @InjectMocks DeploymentStepEventProcessor deploymentStepEventProcessor;
  ...
  public void testTimescaleInvalid() throws SQLException {
    ...
    deploymentStepEventProcessor.processEvent(timeSeriesEventInfo);
  ...
  public void testNullProperties() throws SQLException {
    ...
    timeSeriesEventInfo =
        TimeSeriesEventInfo.builder().longData(new HashMap<>()).booleanData(new HashMap<>()).accountId("abc").build();
    deploymentStepEventProcessor.processEvent(timeSeriesEventInfo);
    verify(timeScaleService, never()).getDBConnection();

    timeSeriesEventInfo =
        TimeSeriesEventInfo.builder().stringData(new HashMap<>()).booleanData(new HashMap<>()).accountId("abc").build();
    deploymentStepEventProcessor.processEvent(timeSeriesEventInfo);
    verify(timeScaleService, never()).getDBConnection();
  }
  ...
  public void testSuccessFullExecution() throws SQLException {
    ...
    TimeSeriesEventInfo timeSeriesEventInfo = TimeSeriesEventInfo.builder()
                                                  ...
                                                  .accountId("abc")
                                                  .build();
    DeploymentStepEventProcessor deploymentStepEventProcessor1 = spy(deploymentStepEventProcessor);
    doNothing().when(deploymentStepEventProcessor1).upsertDataToTimescaleDB(timeSeriesEventInfo, preparedStatement);

    deploymentStepEventProcessor1.processEvent(timeSeriesEventInfo);

    verify(deploymentStepEventProcessor1).upsertDataToTimescaleDB(timeSeriesEventInfo, preparedStatement);
  }
  ...
  public void testWhenThrowsExecution() throws SQLException {
    ...
    doThrow(new SQLException())
        .when(deploymentStepEventProcessor1)
        .upsertDataToTimescaleDB(timeSeriesEventInfo, preparedStatement);

    deploymentStepEventProcessor1.processEvent(timeSeriesEventInfo);

    verify(deploymentStepEventProcessor1, times(MAX_RETRY))
        .upsertDataToTimescaleDB(timeSeriesEventInfo, preparedStatement);
  }
  ...
  public void testUpsertData() throws SQLException {
    ...
    deploymentStepEventProcessor.upsertDataToTimescaleDB(timeSeriesEventInfo, preparedStatement);
