...
@OwnedBy(CDC)
public class JenkinsArtifactStreamYamlHandlerTest extends YamlHandlerTestBase {
  @InjectMocks @Inject private JenkinsArtifactStreamYamlHandler yamlHandler;
  ...
  public void toYaml() {
    JenkinsArtifactStream artifactStream = JenkinsArtifactStream.builder()
  ...
  public void upsertFromYaml() {
    ...
    final ArgumentCaptor<JenkinsArtifactStream> captor = ArgumentCaptor.forClass(JenkinsArtifactStream.class);
    yamlHandler.upsertFromYaml(changeContext, asList(changeContext));
    verify(artifactStreamService).createWithBinding(anyString(), captor.capture(), anyBoolean());
    final JenkinsArtifactStream artifactStream = captor.getValue();
    ...
    assertThat(artifactStream.getArtifactStreamType()).isEqualTo(JENKINS.name());
  }
