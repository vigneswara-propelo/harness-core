...
public class DeploymentEventProcessor implements EventProcessor<TimeSeriesEventInfo> {
  ...
  private static final String fetch_account_executions_deployment_in_interval =
      "SELECT EXECUTIONID,STARTTIME,ENDTIME,ACCOUNTID,APPID,TRIGGERED_BY,TRIGGER_ID,STATUS,SERVICES,WORKFLOWS,CLOUDPROVIDERS,ENVIRONMENTS,PIPELINE,DURATION,ARTIFACTS,ENVTYPES,PARENT_EXECUTION,FAILURE_DETAILS,FAILED_STEP_NAMES,FAILED_STEP_TYPES,STAGENAME,ROLLBACK_DURATION,ON_DEMAND_ROLLBACK, ORIGINAL_EXECUTION_ID, MANUALLY_ROLLED_BACK, INSTANCES_DEPLOYED,TAGS, PARENT_PIPELINE_ID,CREATED_BY_TYPE FROM DEPLOYMENT WHERE ACCOUNTID = ? AND STARTTIME >= ? AND STARTTIME <= ? ORDER BY STARTTIME DESC OFFSET ? LIMIT ?";
