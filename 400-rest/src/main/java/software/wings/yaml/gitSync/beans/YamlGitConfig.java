...
package software.wings.yaml.gitSync.beans;
...
public class YamlGitConfig implements EncryptableSetting, PersistentEntity, UuidAware, CreatedAtAware, CreatedByAware,
  ...
  public software.wings.yaml.gitSync.YamlGitConfig toDTO() {
    software.wings.yaml.gitSync.YamlGitConfig.YamlGitConfigBuilder yamlGitConfigBuilder =
        software.wings.yaml.gitSync.YamlGitConfig.builder()
            .uuid(uuid)
            .createdAt(createdAt)
            ...
            .gitPollingIterator(gitPollingIterator);

    if (createdBy != null) {
      yamlGitConfigBuilder.createdBy(createdBy);
    }

    if (lastUpdatedBy != null) {
      yamlGitConfigBuilder.lastUpdatedBy(lastUpdatedBy);
    }

    if (syncMode != null) {
      yamlGitConfigBuilder.syncMode(software.wings.yaml.gitSync.YamlGitConfig.SyncMode.valueOf(syncMode.name()));
    }

    return yamlGitConfigBuilder.build();
  }
}
