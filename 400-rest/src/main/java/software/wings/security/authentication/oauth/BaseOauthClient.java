...
public class BaseOauthClient {
  ...
  protected void populateEmptyFields(OauthUserInfo oauthUserInfo) {
    ...
    oauthUserInfo.setLogin(isEmpty(oauthUserInfo.getLogin()) ? handle : oauthUserInfo.getEmail());
    oauthUserInfo.setName(isEmpty(oauthUserInfo.getName()) ? handle : oauthUserInfo.getName());
  }
}
