...
import io.harness.statemachine.entity.AnalysisState;
import io.harness.statemachine.entity.AnalysisStateMachine;
import io.harness.statemachine.entity.AnalysisStateMachine.AnalysisStateMachineKeys;
...
public class AnalysisStateMachineServiceImpl implements AnalysisStateMachineService {
  ...
  public void initiateStateMachine(String cvConfigId, AnalysisStateMachine stateMachine) {
    ...
    if (executingStateMachine != null) {
      if (!executingStateMachine.getStatus().equals(AnalysisStatus.SUCCESS)) {
  ...
  public void executeStateMachine(AnalysisStateMachine analysisStateMachine) {
    ...
    if (!analysisStateMachine.getStatus().equals(AnalysisStatus.RUNNING)
    ...
    } else if (nextState.getStatus().equals(AnalysisStatus.CREATED)) {
      nextState.execute();
    }

    injector.injectMembers(nextState);
    if (nextState.getStatus().equals(AnalysisStatus.SUCCESS)) {
  ...
  public void retryStateMachineAfterFailure(AnalysisStateMachine analysisStateMachine) {
    ...
    AnalysisState currentState = analysisStateMachine.getCurrentState();
    injector.injectMembers(currentState);
    if (currentState.getStatus().equals(AnalysisStatus.FAILED)
        || currentState.getStatus().equals(AnalysisStatus.TIMEOUT)) {
