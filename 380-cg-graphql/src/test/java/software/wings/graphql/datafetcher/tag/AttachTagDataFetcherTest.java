...
public class AttachTagDataFetcherTest extends CategoryTest {
  ...
  @InjectMocks @Spy AttachTagDataFetcher attachTagDataFetcher = new AttachTagDataFetcher(tagService);
  ...
  public void test_mutateAndFetch() throws Exception {
    ...
    {
      final QLAttachTagPayload qlAttachTagPayload = attachTagDataFetcher.get(dataFetchingEnvironment);
      ArgumentCaptor<HarnessTagLink> tagLinkArgumentCaptor = ArgumentCaptor.forClass(HarnessTagLink.class);

      verify(tagService, times(1)).attachTag(tagLinkArgumentCaptor.capture());
      ...
      assertThat(tagLink.getEntityId()).isEqualTo("appId");
      assertThat(tagLink.getEntityType().name()).isEqualTo("APPLICATION");
      assertThat(tagLink.getKey()).isEqualTo("tagName");
      assertThat(tagLink.getValue()).isEqualTo("tagValue");
    }
  }
  ...
  @Owner(developers = HINGER)
  @Category(UnitTests.class)
  public void test_mutateAndFetch_WithoutTagValue() throws Exception {
    final DataFetchingEnvironment dataFetchingEnvironment = Mockito.mock(DataFetchingEnvironment.class);
    doReturn(ImmutableMap.of(
                 "clientMutationId", "req1", "entityId", "appId", "entityType", "APPLICATION", "name", "tagName"))
        .when(dataFetchingEnvironment)
        .getArguments();
    doReturn("accountid").when(utils).getAccountId(dataFetchingEnvironment);
    doReturn("appId").when(tagHelper).validateAndFetchAppId("appId", QLEntityType.APPLICATION);

    {
      ...
      assertThat(tagLink.getEntityType().name()).isEqualTo("APPLICATION");
      assertThat(tagLink.getKey()).isEqualTo("tagName");
      // For backward compatibility, convert null to empty string
      assertThat(tagLink.getValue()).isEqualTo("");
    }
  }
}
