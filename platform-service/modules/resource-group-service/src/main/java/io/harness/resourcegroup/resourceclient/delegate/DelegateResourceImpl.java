...
package io.harness.resourcegroup.resourceclient.delegate;
...
@Slf4j
@OwnedBy(DEL)
public class DelegateResourceImpl implements Resource {
  DelegateServiceResourceClient delegateServiceResourceClient;

  @Override
  public List<Boolean> validate(List<String> resourceIds, Scope scope) {
    if (isEmpty(resourceIds)) {
      log.info("Delegate resourceIds is empty.");
    ...
    log.info("Got the response from manager for delegate validation {}", delegateValidityData.toString());

    return delegateValidityData;
  }
  ...
  public Optional<String> getEventFrameworkEntityType() {
    return Optional.of(EventsFrameworkMetadataConstants.DELEGATE_ENTITY);
  }
