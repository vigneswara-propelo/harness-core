...
if [[ "${DEPLOY_MODE}" == "KUBERNETES" ]] || [[ "${DEPLOY_MODE}" == "KUBERNETES_ONPREM" ]]; then
    java $JAVA_OPTS -jar $CAPSULE_JAR /opt/harness/audit-event-streaming-config.yml
else
    java $JAVA_OPTS -jar $CAPSULE_JAR /opt/harness/audit-event-streaming-config.yml > /opt/harness/logs/audit-event-streaming.log 2>&1
fi
