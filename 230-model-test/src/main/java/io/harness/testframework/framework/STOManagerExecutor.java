...
public class STOManagerExecutor {
  ...
  public static void ensureSTOManager(Class<?> clazz) throws IOException {
    if (!isHealthy()) {
  ...
  public static void executeLocalManager(String verb, Class<?> clazz) throws IOException {
    ...
    if (FileIo.acquireLock(lockfile, waiting)) {
      try {
        ...
        Poller.pollFor(waiting, ofSeconds(2), STOManagerExecutor::isHealthy);
  ...
  private static boolean isHealthy() {
    try {
      ...
      Setup.sto().config(config).when().get("/health").then().statusCode(HttpStatus.SC_OK);
