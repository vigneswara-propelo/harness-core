...
public class CommandYamlResource {
   ...
   */
  @Inject
  public CommandYamlResource(AppService appService, ServiceResourceService serviceResourceService,
      CommandService commandService, YamlHistoryService yamlHistoryService) {
   ...
   */
  @GET
  @Path("/{accountId}/{appId}/{serviceId}/{serviceCommandId}")
  @Timed
  @ExceptionMetered
  public RestResponse<YamlPayload> get(@PathParam("appId") String appId, @PathParam("serviceId") String serviceId,
    ...
    /*
    Command command = commandService.getCommand(appId, serviceCommandId,  version);
    ...
    YamlHelper.addResponseMessage(rr, ErrorCode.GENERAL_YAML_ERROR, ResponseTypeEnum.ERROR, "Service with this
    serviceId: '" + serviceId + "' was not found!");
  ...
  public RestResponse<Service> update(@PathParam("appId") String appId, @PathParam("serviceId") String serviceId,
    ...
    commands to add list for (String sc : beforeServiceCommands) { serviceCommandsToDelete.add(sc);
    ...
    list for (ConfigVarYaml cv : beforeConfigVars) { configVarsToDelete.add(cv); configVarsToAdd.remove(cv);
            }
          }
        }
        ...
        if (configVarsToAdd != null) {
          ...
          for (ConfigVarYaml cv : configVarsToAdd) {
            ServiceVariable savedServiceVariable = serviceVariableService.save(createNewServiceVariable(appId,
    service.getUuid(), cv));
          }
        }
        ...
        try {
          ...
          e.printStackTrace();
          YamlHelper.addResponseMessage(rr, ErrorCode.GENERAL_YAML_ERROR, ResponseTypeEnum.ERROR, "The ArtifactType: '"
    + artifactTypeStr + "' is not found in the ArtifactType Enum!"); return rr;
        }
        ...
        if (service != null) {
          ...
          YamlVersion
              beforeYamLVersion =
    aYamlVersion().withAccountId(accountId).withEntityId(accountId).withType(YamlVersion.Type.SERVICE).withYaml(beforeYaml).build();
