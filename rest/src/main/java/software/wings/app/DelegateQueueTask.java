...
public class DelegateQueueTask implements Runnable {
  ...
  private void reBroadcastQueuedTask() {
  ...
  private void updateStatusToErrorForQueuedTimedOutTasks() {
    ...
    if (!queuedTimedOutTasks.isEmpty()) {
      ...
      queuedTimedOutTasks.forEach(delegateTask -> {
        ...
        if (updatedDelegateTask == null) {
          ...
          logger.error("Queued delegate task {} with type {} has no wait ID. Nothing to notify", delegateTask.getUuid(),
  ...
  private void updateStatusToFailedForLongRunningTimedOutTasks() {
  ...
  private void releaseAcquiredButNotStartedTasks() {
