...
public void shouldRunDelegate() throws IOException, JSONException, TimeoutException, InterruptedException {
  ...
  new ProcessExecutor()
      .command("/bin/sh", "-c",
          "unzip delegate.zip && cd harness-delegate && sed -i '' -e 's/doUpgrade: true/doUpgrade: false/' run.sh")
      .readOutput(true)
      .execute()
      .getOutput()
      .getLines()
  ...
  new ProcessExecutor()
      .command("/bin/sh", "-c",
          "cd harness-delegate && rm delegate.jar config-delegate.yml && sed -i '' -e 's/REMOTE_DELEGATE_URL=.*/REMOTE_DELEGATE_URL=https:\\/\\/s3.amazonaws.com\\/wingsdelegates\\/jobs\\/test-delegate\\/delegate.jar/' run.sh && sed -i '' -e 's/doUpgrade: false/doUpgrade: true/' run.sh")
      .readOutput(true)
      .execute()
      .getOutput()
      .getLines()
