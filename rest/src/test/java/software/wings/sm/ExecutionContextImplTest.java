...
public class ExecutionContextImplTest extends WingsBaseTest {
  ...
  @Test
  public void shouldRenderExpressionForExecCommand1() {
    StateExecutionInstance stateExecutionInstance = new StateExecutionInstance();
    stateExecutionInstance.setStateName("abc");
    ExecutionContextImpl context = new ExecutionContextImpl(stateExecutionInstance);
    injector.injectMembers(context);
    Application app = anApplication().withName("a\\b").withAccountId(ACCOUNT_ID).build();
    app = appService.save(app);

    Environment env = anEnvironment().withAppId(app.getUuid()).withName("a\"b").build();
    env = environmentService.save(env);
    ...
    String expr = "echo \"${app.name}\" echo \"${env.name}\"";
    String commandText = context.renderExpressionForExecCommand(expr);
    assertThat(commandText).isEqualTo("echo \"a\\\\b\" echo \"a\\\"b\"");
  }

  @Test
  public void shouldRenderExpressionForExecCommand2() {
    StateExecutionInstance stateExecutionInstance = new StateExecutionInstance();
    stateExecutionInstance.setStateName("abc");
    ExecutionContextImpl context = new ExecutionContextImpl(stateExecutionInstance);
    injector.injectMembers(context);
  ...
  @Test
  public void shouldRenderExpressionForExecCommand3() {
    StateExecutionInstance stateExecutionInstance = new StateExecutionInstance();
    stateExecutionInstance.setStateName("abc");
    ExecutionContextImpl context = new ExecutionContextImpl(stateExecutionInstance);
    injector.injectMembers(context);
