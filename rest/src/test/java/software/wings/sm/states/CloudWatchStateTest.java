...
public class CloudWatchStateTest extends WingsBaseTest {
  ...
  @InjectMocks @Spy private CloudWatchState cloudWatchState = new CloudWatchState(StateType.CLOUD_WATCH.name());
    ...
    when(settingsService.get(Base.GLOBAL_APP_ID, SETTING_ID))
        .thenReturn(
            aSettingAttribute()
                ...
                .build());
    when(cloudWatchState.getAmazonCloudWatchClient(any(BasicAWSCredentials.class))).thenReturn(amazonCloudWatchClient);
  ...
  public void shouldExecute() {
    ...
    assertThat(getMetricStatisticsRequest.getNamespace()).isEqualTo("AWS/EC2");
