...
public class WorkflowExecutionServiceImplTest extends WingsBaseTest {
  ...
  //
  //    Host host1 = wingsPersistence.saveAndGet(Host.class,
  //    aHost().withAppId(app.getUuid()).withEnvId(env.getUuid()).withHostName("host1").build()); Host host2 =
  //    wingsPersistence.saveAndGet(Host.class,
  //    aHost().withAppId(app.getUuid()).withEnvId(env.getUuid()).withHostName("host2").build());
  //
  //    Service service1 =
  ...
  //    Service service2 =
  //        wingsPersistence.saveAndGet(Service.class,
  //        aService().withUuid(UUIDGenerator.getUuid()).withName("svc2").withAppId(app.getUuid()).build());
  //    ServiceTemplate serviceTemplate1 = wingsPersistence.saveAndGet(ServiceTemplate.class,
  //        aServiceTemplate().withAppId(app.getUuid()).withEnvId(env.getUuid()).withServiceId(service1.getUuid()).withName(service1.getName())
  //            .withAppId(app.getUuid()).withDescription("TEMPLATE_DESCRIPTION").build());
  //    serviceTemplate1.setService(service1);
  //    ServiceTemplate serviceTemplate2 = wingsPersistence.saveAndGet(ServiceTemplate.class,
  //        aServiceTemplate().withAppId(app.getUuid()).withEnvId(env.getUuid()).withServiceId(service2.getUuid()).withName(service2.getName())
  //            .withAppId(app.getUuid()).withDescription("TEMPLATE_DESCRIPTION").build());
  ...
  //
  //    Workflow orchestration = anWorkflow().withAppId(app.getUuid()).withName("workflow1").withDescription("Sample
  //    Workflow").withGraph(graph)
  //        .withWorkflowType(WorkflowType.ORCHESTRATION).withTargetToAllEnv(true).build();
  //    orchestration = workflowService.createWorkflow(Workflow.class, orchestration);
  ...
  //        .triggerWorkflowExecution(app.getUuid(), env.getUuid(), orchestration.getUuid(), executionArgs, callback);
  //    callback.await();
  //
  //
  ...
  //    assertThat(execution).isNotNull().extracting("uuid", "status").containsExactly(executionId,
  //    ExecutionStatus.SUCCESS); assertThat(execution.getExecutionNode()).isNotNull().extracting("name", "type",
  //    "status").containsExactly("Repeat By Services", "REPEAT", "SUCCESS");
  ...
  //    assertThat(svcElements).isNotNull().hasSize(2).extracting("name").contains(service1.getName(),
  //    service2.getName()); assertThat(svcElements).extracting("type").contains("ELEMENT", "ELEMENT");
  //
  ...
  //    assertThat(repeatInstance).isNotNull().hasSize(2).extracting("name").contains("RepeatByInstances",
  //    "RepeatByInstances"); assertThat(repeatInstance).extracting("type").contains("REPEAT", "REPEAT");
  //
  //    List<Node> instSuccessWait = repeatInstance.stream().map(Node::getNext).collect(Collectors.toList());
  //    assertThat(instSuccessWait).isNotNull().hasSize(2).extracting("name").contains("instSuccessWait",
  //    "instSuccessWait"); assertThat(instSuccessWait).extracting("type").contains("WAIT", "WAIT");
  //
  //    List<Node> instRepeatElements = repeatInstance.stream().map(Node::getGroup).flatMap(g ->
  //    g.getElements().stream()).collect(Collectors.toList());
  ...
  //    assertThat(instRepeatWait).isNotNull().hasSize(4).extracting("name").contains("instRepeatWait",
  //    "instRepeatWait", "instRepeatWait", "instRepeatWait");
  ...
  //        "workflowId", orchestration.getUuid()))));
  ...
  //    Environment env = wingsPersistence.saveAndGet(Environment.class,
  //    Builder.anEnvironment().withAppId(appId).build()); triggerWorkflow(appId, env);
  //  }
  //
  //  /**
  ...
  //    String appId = app.getUuid();
  //    Environment env = wingsPersistence.saveAndGet(Environment.class,
  //    Builder.anEnvironment().withAppId(appId).build());
  //
  ...
  //    String appId = app.getUuid();
  //    Environment env = wingsPersistence.saveAndGet(Environment.class,
  //    Builder.anEnvironment().withAppId(appId).build()); triggerWorkflow(appId, env); WorkflowExecution
  //    workflowExecution = wingsPersistence.get(WorkflowExecution.class, new PageRequest<>());
  ...
  //    Workflow orchestration = createExecutableWorkflow(appId, env);
  //    ExecutionArgs executionArgs = new ExecutionArgs();
  //
  //    WorkflowExecutionUpdateMock callback = new WorkflowExecutionUpdateMock();
  //    WorkflowExecution execution = ((WorkflowExecutionServiceImpl) workflowExecutionService)
  //        .triggerWorkflowExecution(appId, env.getUuid(), orchestration.getUuid(), executionArgs, callback);
  //    callback.await();
  //
  //    assertThat(execution).isNotNull();
  //    String executionId = execution.getUuid();
  ...
  //
  //    Graph graph = aGraph().addNodes(
  //        aNode().withId("n1").withName("wait").withX(200).withY(50).withType(StateType.WAIT.name()).addProperty("duration",
  //        1l).withOrigin(true).build())
  //        .addNodes(aNode().withId("n2").withName("email").withX(250).withY(50).withType(StateType.EMAIL.name()).addProperty("toAddress",
  //        "a@b.com")
  //            .addProperty("subject",
  //            "testing").build()).addLinks(aLink().withId("l1").withFrom("n1").withTo("n2").withType("success").build()).build();
  //
  //    Workflow orchestration = anWorkflow().withAppId(appId).withName("workflow1").withDescription("Sample
  //    Workflow").withGraph(graph)
  //        .withWorkflowType(WorkflowType.ORCHESTRATION).withTargetToAllEnv(true).build();
  //    orchestration = workflowService.createWorkflow(Workflow.class, orchestration);
  //    assertThat(orchestration).isNotNull();
  ...
  //    PageResponse<Workflow> res = workflowService.listWorkflow(pageRequest, null);
  //
  ...
  //
  //    Graph graph =
  //        aGraph().addNodes(aNode().withId("wait1").withOrigin(true).withName("wait1").withType(StateType.WAIT.name()).addProperty("duration",
  //        1).build())
  //            .addNodes(aNode().withId("pause1").withName("pause1").withType(StateType.PAUSE.name()).addProperty("toAddress",
  //            "to1").build())
  //            .addNodes(aNode().withId("wait2").withName("wait2").withType(StateType.WAIT.name()).addProperty("duration",
  //            1).build())
  //            .addLinks(aLink().withId("l1").withFrom("wait1").withTo("pause1").withType("success").build())
  //            .addLinks(aLink().withId("l2").withFrom("pause1").withTo("wait2").withType("success").build()).build();
  //
  //    Workflow orchestration = anWorkflow().withAppId(app.getUuid()).withName("workflow1").withDescription("Sample
  //    Workflow").withGraph(graph)
  //        .withWorkflowType(WorkflowType.ORCHESTRATION).withTargetToAllEnv(true).build();
  ...
  //
  //    assertThat(execution.getExecutionNode()).isNotNull().hasFieldOrPropertyWithValue("name",
  //    "wait1").hasFieldOrPropertyWithValue("status", "SUCCESS");
  //    assertThat(execution.getExecutionNode().getNext()).isNotNull().hasFieldOrPropertyWithValue("name", "pause1")
  //        .hasFieldOrPropertyWithValue("status", "PAUSED");
  //
  //    ExecutionInterrupt executionInterrupt =
  //        ExecutionInterrupt.Builder.aWorkflowExecutionInterrupt().withAppId(app.getUuid()).withEnvId(env.getUuid()).withExecutionUuid(executionId)
  //            .withStateExecutionInstanceId(execution.getExecutionNode().getNext().getId()).withExecutionInterruptType(ExecutionInterruptType.RESUME).build();
  ...
  //    assertThat(execution.getExecutionNode().getNext()).isNotNull().hasFieldOrPropertyWithValue("name", "pause1")
  //        .hasFieldOrPropertyWithValue("status", "SUCCESS");
  //    assertThat(execution.getExecutionNode().getNext().getNext()).isNotNull().hasFieldOrPropertyWithValue("name",
  //    "wait2")
  //        .hasFieldOrPropertyWithValue("status", "SUCCESS");
  //
  //  }
  ...
  //
  //    Graph graph =
  //    aGraph().addNodes(aNode().withId("RepeatByServices").withOrigin(true).withName("RepeatByServices").withType(StateType.REPEAT.name())
  //        .addProperty("repeatElementExpression", "${services()}").addProperty("executionStrategy",
  //        ExecutionStrategy.PARALLEL).build())
  //        .addNodes(aNode().withId("wait1").withName("wait1").withType(StateType.WAIT.name()).addProperty("duration",
  //        1).build())
  //        .addNodes(aNode().withId("wait2").withName("wait2").withType(StateType.WAIT.name()).addProperty("duration",
  //        1).build())
  //        .addLinks(aLink().withId("l1").withFrom("RepeatByServices").withTo("wait1").withType("repeat").build())
  //        .addLinks(aLink().withId("l2").withFrom("wait1").withTo("wait2").withType("success").build()).build();
  //
  //    Workflow orchestration = anWorkflow().withAppId(app.getUuid()).withName("workflow1").withDescription("Sample
  //    Workflow").withGraph(graph)
  //        .withWorkflowType(WorkflowType.ORCHESTRATION).withTargetToAllEnv(true).build();
  ...
  //
  //    ExecutionInterrupt executionInterrupt =
  //        ExecutionInterrupt.Builder.aWorkflowExecutionInterrupt().withAppId(app.getUuid()).withExecutionInterruptType(ExecutionInterruptType.PAUSE_ALL)
  //            .withExecutionUuid(executionId).withEnvId(env.getUuid()).build();
  ...
  //
  //    List<Node> wait1List =
  //        execution.getExecutionNode().getGroup().getElements().stream().filter(n -> n.getNext() !=
  //        null).map(Node::getNext).collect(Collectors.toList());
  //    List<Node> wait2List = wait1List.stream().filter(n -> n.getNext() !=
  //    null).map(Node::getNext).collect(Collectors.toList());
  //
  //    assertThat(execution).isNotNull().extracting("uuid", "status").containsExactly(executionId,
  //    ExecutionStatus.PAUSED); assertThat(execution.getExecutionNode()).extracting("name", "type",
  //    "status").containsExactly("RepeatByServices", "REPEAT", "RUNNING"); assertThat(wait1List).filteredOn("name",
  //    "wait1").hasSize(2).allMatch(n -> "WAIT".equals(n.getType()) && "SUCCESS".equals(n.getStatus()));
  //    assertThat(wait2List).filteredOn("name", "wait2").hasSize(2).allMatch(n -> "WAIT".equals(n.getType()) &&
  //    "PAUSED".equals(n.getStatus()));
  //
  //    executionInterrupt =
  //    ExecutionInterrupt.Builder.aWorkflowExecutionInterrupt().withAppId(app.getUuid()).withExecutionInterruptType(ExecutionInterruptType.RESUME_ALL)
  //        .withExecutionUuid(executionId).withEnvId(env.getUuid()).build();
  //    executionInterrupt = workflowExecutionService.triggerExecutionInterrupt(executionInterrupt);
  //    assertThat(executionInterrupt).isNotNull().hasFieldOrProperty("uuid");
  //
  ...
  //    ExecutionStatus.SUCCESS);
  //
  //    wait1List = execution.getExecutionNode().getGroup().getElements().stream().filter(n -> n.getNext() !=
  //    null).map(Node::getNext).collect(Collectors.toList()); wait2List = wait1List.stream().filter(n -> n.getNext() !=
  //    null).map(Node::getNext).collect(Collectors.toList());
  //
  //    assertThat(execution.getExecutionNode()).extracting("name", "type",
  //    "status").containsExactly("RepeatByServices", "REPEAT", "SUCCESS"); assertThat(wait1List).filteredOn("name",
  //    "wait1").hasSize(2).allMatch(n -> "WAIT".equals(n.getType()) && "SUCCESS".equals(n.getStatus()));
  //    assertThat(wait2List).filteredOn("name", "wait2").hasSize(2).allMatch(n -> "WAIT".equals(n.getType()) &&
  //    "SUCCESS".equals(n.getStatus()));
  //  }
  //
  //  /**
  ...
  //    Application app = wingsPersistence.saveAndGet(Application.class, anApplication().withName("App1").build());
  //    Environment env = wingsPersistence.saveAndGet(Environment.class,
  //    Builder.anEnvironment().withAppId(app.getUuid()).build()); ExecutionInterrupt executionInterrupt =
  //        ExecutionInterrupt.Builder.aWorkflowExecutionInterrupt().withAppId(app.getUuid()).withExecutionInterruptType(ExecutionInterruptType.PAUSE).withEnvId(env.getUuid())
  //            .withExecutionUuid(UUIDGenerator.getUuid()).build();
  //    try {
  //      executionInterrupt = workflowExecutionService.triggerExecutionInterrupt(executionInterrupt);
  //      failBecauseExceptionWasNotThrown(WingsException.class);
  //    } catch (WingsException exception) {
  //      assertThat(exception.getParams().values()).doesNotContainNull();
  //      assertThat(exception.getParams().values().iterator().next()).isInstanceOf(String.class).asString().startsWith("no
  //      workflowExecution for executionUuid"); assertThat(exception).hasMessage(ErrorCode.INVALID_ARGUMENT.getCode());
  //    }
  //  }
  //
  //
  ...
  //    ExecutionInterrupt executionInterrupt =
  //        ExecutionInterrupt.Builder.aWorkflowExecutionInterrupt().withAppId(app.getUuid()).withEnvId(env.getUuid()).withExecutionUuid(executionId)
  //            .withStateExecutionInstanceId(execution.getExecutionNode().getNext().getId()).withExecutionInterruptType(ExecutionInterruptType.ABORT).build();
  //    workflowExecutionService.triggerExecutionInterrupt(executionInterrupt);
  //    callback.await();
  //
  //    execution = workflowExecutionService.getExecutionDetails(app.getUuid(), executionId);
  ...
  //
  //    ExecutionInterrupt executionInterrupt =
  //        ExecutionInterrupt.Builder.aWorkflowExecutionInterrupt().withAppId(app.getUuid()).withExecutionInterruptType(ExecutionInterruptType.ABORT_ALL)
  //            .withExecutionUuid(executionId).withEnvId(env.getUuid()).build();
  //    executionInterrupt = workflowExecutionService.triggerExecutionInterrupt(executionInterrupt);
  //    assertThat(executionInterrupt).isNotNull().hasFieldOrProperty("uuid");
  //
  ...
  //    ExecutionStatus.ABORTED);
  //
  //    assertThat(execution.getExecutionNode()).isNotNull().hasFieldOrPropertyWithValue("name", "RepeatByServices")
  //        .hasFieldOrPropertyWithValue("status", "ABORTED");
  //    assertThat(execution.getExecutionNode().getGroup()).isNotNull();
  //    assertThat(execution.getExecutionNode().getGroup().getElements()).isNotNull().hasSize(2).extracting("next").doesNotContainNull().extracting("name")
  //        .contains("wait1", "wait1");
  //    assertThat(execution.getExecutionNode().getGroup().getElements()).isNotNull().hasSize(2).extracting("next").doesNotContainNull().extracting("status")
  ...
  //    Builder.anEnvironment().withAppId(app.getUuid()).build());
  //
  //    Host applicationHost1 = wingsPersistence.saveAndGet(Host.class,
  //    aHost().withAppId(app.getAppId()).withEnvId(env.getUuid()).withHostName("host1").build()); Host applicationHost2
  //    = wingsPersistence.saveAndGet(Host.class,
  //    aHost().withAppId(app.getAppId()).withEnvId(env.getUuid()).withHostName("host2").build());
  //
  //    Service service =
  //        wingsPersistence.saveAndGet(Service.class,
  //        aService().withUuid(UUIDGenerator.getUuid()).withName("svc1").withAppId(app.getUuid()).build());
  ...
  //    aGraph().addNodes(aNode().withId("RepeatByServices").withOrigin(true).withName("RepeatByServices").withType(StateType.REPEAT.name())
  //        .addProperty("repeatElementExpression", "${services()}").addProperty("executionStrategy",
  //        ExecutionStrategy.PARALLEL).build()).addNodes(
  //        aNode().withId("RepeatByInstances").withName("RepeatByInstances").withType(StateType.REPEAT.name())
  //            .addProperty("repeatElementExpression", "${instances()}").addProperty("executionStrategy",
  //            ExecutionStrategy.SERIAL).build())
  //        .addNodes(aNode().withId("install").withName("install").withType(StateType.COMMAND.name()).addProperty("command",
  //        "install").build())
  //        .addLinks(aLink().withId("l1").withFrom("RepeatByServices").withTo("RepeatByInstances").withType("repeat").build())
  //        .addLinks(aLink().withId("l2").withFrom("RepeatByInstances").withTo("install").withType("repeat").build()).build();
  //
  //    Workflow orchestration = anWorkflow().withAppId(app.getUuid()).withName("workflow1").withDescription("Sample
  //    Workflow").withGraph(graph)
  //        .withWorkflowType(WorkflowType.ORCHESTRATION).withTargetToAllEnv(true).build();
  ...
  //      installNodes =
  //          execution.getExecutionNode().getGroup().getElements().stream().filter(n -> n.getNext() !=
  //          null).map(Node::getNext).filter(n -> n.getGroup() != null)
  //              .map(Node::getGroup).filter(g -> g.getElements() != null).flatMap(g ->
  //              g.getElements().stream()).filter(n -> n.getNext() != null)
  //              .map(Node::getNext).collect(Collectors.toList());
  //      paused =
  //          !installNodes.stream().filter(n -> n.getStatus() != null &&
  //          n.getStatus().equals(ExecutionStatus.PAUSED_ON_ERROR.name())).collect(Collectors.toList())
  //              .isEmpty();
  //    } while (!paused && i < 5);
  //
  //    execution = workflowExecutionService.getExecutionDetails(app.getUuid(), executionId);
  //    assertThat(execution).isNotNull().hasFieldOrPropertyWithValue("uuid",
  //    executionId).hasFieldOrPropertyWithValue("status", ExecutionStatus.PAUSED);
  //    assertThat(installNodes).isNotNull().doesNotContainNull().filteredOn("name",
  //    "install").hasSize(1).extracting("status")
  //        .containsExactly(ExecutionStatus.PAUSED_ON_ERROR.name());
  //
  //    Node installNode = installNodes.get(0);
  //    ExecutionInterrupt executionInterrupt =
  //        ExecutionInterrupt.Builder.aWorkflowExecutionInterrupt().withAppId(app.getUuid()).withEnvId(env.getUuid()).withExecutionUuid(executionId)
  //            .withStateExecutionInstanceId(installNode.getId()).withExecutionInterruptType(ExecutionInterruptType.RESUME).build();
  //    workflowExecutionService.triggerExecutionInterrupt(executionInterrupt);
  //
  //    i = 0;
  //    installNodes = null;
  ...
  //    executionId).hasFieldOrPropertyWithValue("status", ExecutionStatus.PAUSED);
  //    assertThat(installNodes).isNotNull().doesNotContainNull().filteredOn("name",
  //    "install").hasSize(2).extracting("status")
  ...
  //    execution = workflowExecutionService.getExecutionDetails(app.getUuid(), executionId);
  //    assertThat(execution).isNotNull().hasFieldOrPropertyWithValue("uuid",
  //    executionId).hasFieldOrPropertyWithValue("status", ExecutionStatus.SUCCESS); installNodes =
  //        execution.getExecutionNode().getGroup().getElements().stream().filter(n -> n.getNext() !=
  //        null).map(Node::getNext).filter(n -> n.getGroup() != null)
  //            .map(Node::getGroup).filter(g -> g.getElements() != null).flatMap(g ->
  //            g.getElements().stream()).filter(n -> n.getNext() != null).map(Node::getNext)
  ...
  //
  //    Host host1 = wingsPersistence.saveAndGet(Host.class,
  //    aHost().withAppId(app.getUuid()).withEnvId(env.getUuid()).withHostName("host1").build());
  //
  //
  //    Service service =
  //        wingsPersistence.saveAndGet(Service.class,
  ...
  //    ExecutionInterrupt executionInterrupt =
  //        ExecutionInterrupt.Builder.aWorkflowExecutionInterrupt().withAppId(app.getUuid()).withEnvId(env.getUuid()).withExecutionUuid(executionId)
  //            .withStateExecutionInstanceId(installNode.getId()).withExecutionInterruptType(ExecutionInterruptType.RETRY).build();
  //    workflowExecutionService.triggerExecutionInterrupt(executionInterrupt);
  //
  //    i = 0;
  //    installNodes = null;
  ...
  //    execution = workflowExecutionService.getExecutionDetails(app.getUuid(), executionId);
  //    assertThat(execution).isNotNull().hasFieldOrPropertyWithValue("uuid",
  //    executionId).hasFieldOrPropertyWithValue("status", ExecutionStatus.SUCCESS);
  //
  //    installNodes =
  //        execution.getExecutionNode().getGroup().getElements().stream().filter(n -> n.getNext() !=
  //        null).map(Node::getNext).filter(n -> n.getGroup() != null)
  ...
  //  @Test
  //  @Ignore
  //  public void shouldTriggerWorkflowWorkflow() throws InterruptedException {
  //    Application app = wingsPersistence.saveAndGet(Application.class, anApplication().withName("App1").build());
  //    Environment env = wingsPersistence.saveAndGet(Environment.class,
  //    Builder.anEnvironment().withName(ENV_NAME).withAppId(app.getUuid()).build());
  //
  //    Service service =
  //        wingsPersistence.saveAndGet(Service.class,
  //        aService().withUuid(UUIDGenerator.getUuid()).withName("svc1").withAppId(app.getUuid()).build());
  ...
  //    serviceTemplate.setService(service);
  //
  //    SettingAttribute computeProvider = wingsPersistence
  //        .saveAndGet(SettingAttribute.class,
  //        aSettingAttribute().withAppId(app.getUuid()).withValue(aPhysicalDataCenterConfig().build()).build());
  //
  //    infrastructureMappingService.save(PhysicalInfrastructureMapping.Builder.aPhysicalInfrastructureMapping().withAppId(app.getUuid()).withEnvId(env.getUuid())
  //        .withHostNames(Lists.newArrayList("host1")).withServiceTemplateId(serviceTemplate.getUuid()).withComputeProviderSettingId(computeProvider.getUuid())
  //        .build());
  //
  //    triggerWorkflowWorkflow(app.getAppId(), env, service, computeProvider);
  //  }
  //
  //  /**
  //   * Trigger orchestration.
  ...
  //   * @throws InterruptedException the interrupted exception
  //   */
  //  public String triggerWorkflowWorkflow(String appId, Environment env, Service service, SettingAttribute
  //  computeProvider) throws InterruptedException {
  //    CanaryWorkflowWorkflow orchestration = createWorkflowWorkflow(appId, env, service, computeProvider);
  //    ExecutionArgs executionArgs = new ExecutionArgs();
  //
  //    WorkflowExecutionUpdateMock callback = new WorkflowExecutionUpdateMock();
  //    WorkflowExecution execution = ((WorkflowExecutionServiceImpl) workflowExecutionService)
  //        .triggerWorkflowWorkflowExecution(appId, env.getUuid(), orchestration.getUuid(), executionArgs, callback);
  //    callback.await();
  //
  //    assertThat(execution).isNotNull();
  //    String executionId = execution.getUuid();
  ...
  //  }
  //
  //  private CanaryWorkflowWorkflow createWorkflowWorkflow(String appId, Environment env, Service service,
  //  SettingAttribute computeProvider) {
  //
  //    CanaryWorkflowWorkflow orchestrationWorkflow =
  //        anWorkflowWorkflow().withAppId(appId).withEnvId(env.getUuid()).withWorkflowWorkflowType(WorkflowWorkflowType.CANARY)
  //            .withPreDeploymentSteps(aPhaseStep(PhaseStepType.PRE_DEPLOYMENT,
  //            Constants.PRE_DEPLOYMENT).build()).addWorkflowPhases(
  //            aWorkflowPhase().withName("Phase1").withServiceId(service.getUuid()).withDeploymentType(SSH)
  //                .build()).withPostDeploymentSteps(aPhaseStep(PhaseStepType.POST_DEPLOYMENT,
  //                Constants.POST_DEPLOYMENT).build()).build();
  //
  //    CanaryWorkflowWorkflow orchestrationWorkflow2 = workflowService.createWorkflowWorkflow(orchestrationWorkflow);
  //    assertThat(orchestrationWorkflow2).isNotNull().hasFieldOrProperty("uuid").hasFieldOrProperty("preDeploymentSteps").hasFieldOrProperty("postDeploymentSteps")
  //        .hasFieldOrProperty("graph");
  //
  //    CanaryWorkflowWorkflow orchestrationWorkflow3 =
  ...
  //
  //    workflowService.updateWorkflowPhase(orchestrationWorkflow2.getAppId(), orchestrationWorkflow2.getUuid(),
  //    workflowPhase);
  //
  //    CanaryWorkflowWorkflow orchestrationWorkflow4 =
  //        workflowService.readWorkflow(orchestrationWorkflow2.getAppId(), orchestrationWorkflow2.getUuid());
  //
