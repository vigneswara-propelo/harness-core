...
//    when(artifactService.create(any(Artifact.class))).thenReturn(newArtifact);
//
//    Artifact collectedArtifact = artifactCollectionService.collectArtifact(APP_ID, ARTIFACT_STREAM_ID,
//    s3BuildDetails); assertThat(collectedArtifact).isNotNull();
...
//
//    List<Artifact> collectedArtifacts = artifactCollectionService.collectNewArtifactsAsync(APP_ID,
//    ARTIFACT_STREAM_ID); assertThat(collectedArtifacts).isNotNull().size().isEqualTo(1); Artifact collectedArtifact =
//    collectedArtifacts.get(0); assertThat(collectedArtifact).isNotNull();
...
//    Artifact newArtifact = ArtifactCollectionUtil.getArtifact(jenkinsArtifactStream, jenkinsBuildDetails);
//    when(artifactService.create(any(Artifact.class))).thenReturn(newArtifact);
//    when(buildSourceService.getBuilds(APP_ID, ARTIFACT_STREAM_ID,
//    SETTING_ID)).thenReturn(asList(jenkinsBuildDetails));
//
//    when(buildSourceService.getLastSuccessfulBuild(APP_ID, ARTIFACT_STREAM_ID, SETTING_ID))
