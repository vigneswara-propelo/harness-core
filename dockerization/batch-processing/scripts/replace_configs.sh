...
if [[ "" != "$CE_NG_SERVICE_SECRET" ]]; then
  export NEXT_GEN_MANAGER_SECRET; yq -i '.ceNgServiceSecret=env(NEXT_GEN_MANAGER_SECRET)' $CONFIG_FILE
fi
...
replace_key_value awsServiceEndpointUrls.s3EndPointUrl "$AWS_SERVICE_ENDPOINT_URLS_S3_ENDPOINT_URL"
replace_key_value awsServiceEndpointUrls.stsEndPointUrl "$AWS_SERVICE_ENDPOINT_URLS_STS_ENDPOINT_URL"
replace_key_value awsServiceEndpointUrls.ec2EndPointUrl "$AWS_SERVICE_ENDPOINT_URLS_EC2_ENDPOINT_URL"
