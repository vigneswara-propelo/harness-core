...
if [[ "${DEPLOY_MODE}" == "AWS" ]]; then
    ...
    sed -i "s|<property name=\"hazelcast.discovery.enabled\">false|<property name=\"hazelcast.discovery.enabled\">true|" /opt/harness/hazelcast.xml
    sed -i "s|<discovery-strategy enabled=\"false\" class=\"com.hazelcast.kubernetes.HazelcastKubernetesDiscoveryStrategy\">|<discovery-strategy enabled=\"true\" class=\"com.hazelcast.kubernetes.HazelcastKubernetesDiscoveryStrategy\">|" /opt/harness/hazelcast.xml
fi