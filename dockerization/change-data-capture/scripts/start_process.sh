...
if [[ "${DEPLOY_MODE}" == "KUBERNETES" ]] || [[ "${DEPLOY_MODE}" == "KUBERNETES_ONPREM" ]]; then
    java $JAVA_OPTS -jar $CAPSULE_JAR server /opt/harness/config.yml
else
    java $JAVA_OPTS -jar $CAPSULE_JAR server /opt/harness/config.yml > /opt/harness/logs/change-data-capture.log 2>&1
fi
