...
if [[ ( -z "$DISABLE_CLIENT_TOOLS") || ("$DISABLE_CLIENT_TOOLS" = "false") ]]; then
  ...
  && mkdir -m 777 -p client-tools/scm/9f1eaa9f \
  && curl -s -L -o client-tools/scm/9f1eaa9f/scm https://app.harness.io/public/shared/tools/scm/release/9f1eaa9f/bin/linux/$ARCH/scm
else
