...
@RecasterAlias("io.harness.cdng.manifest.yaml.kinds.AwsLambdaDefinitionManifest")
public class AwsLambdaDefinitionManifest implements ManifestAttributes, Visitable {
  ...
  public ManifestAttributeStepParameters getManifestAttributeStepParameters() {
    return new AwsLambdaDefinitionManifestStepParameters(
        identifier, StoreConfigWrapperParameters.fromStoreConfigWrapper(store.getValue()));
  }

  @Override
  public ManifestAttributes applyOverrides(ManifestAttributes overrideConfig) {
    AwsLambdaDefinitionManifest awsLambdaDefinitionManifest = (AwsLambdaDefinitionManifest) overrideConfig;
    AwsLambdaDefinitionManifest resultantManifest = this;
    if (awsLambdaDefinitionManifest.getStore() != null && awsLambdaDefinitionManifest.getStore().getValue() != null) {
      resultantManifest = resultantManifest.withStore(ParameterField.createValueField(
          store.getValue().applyOverrides(awsLambdaDefinitionManifest.getStore().getValue())));
    }
    return resultantManifest;
  }

  @Value
  public static class AwsLambdaDefinitionManifestStepParameters implements ManifestAttributeStepParameters {
    String identifier;
