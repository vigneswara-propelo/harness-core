...
public class CostEventServiceImpl implements CostEventService {
  ...
  static final String INSERT_STATEMENT =
      "INSERT INTO COST_EVENT_DATA (STARTTIME, ACCOUNTID, SETTINGID, CLUSTERID, CLUSTERTYPE, INSTANCEID, INSTANCETYPE, APPID, SERVICEID, ENVID, CLOUDPROVIDERID, DEPLOYMENTID, CLOUDPROVIDER, EVENTDESCRIPTION, COSTEVENTTYPE, COSTEVENTSOURCE, NAMESPACE, WORKLOADNAME, WORKLOADTYPE, CLOUDSERVICENAME, TASKID, LAUNCHTYPE, BILLINGAMOUNT) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT DO NOTHING ";

  static final String UPDATE_DEPLOYMENT_STATEMENT =
      "UPDATE COST_EVENT_DATA SET SETTINGID=?, CLUSTERID=?, CLUSTERTYPE=?, CLOUDPROVIDER=?, NAMESPACE=?, WORKLOADNAME=?, WORKLOADTYPE=?, CLOUDSERVICENAME=?, TASKID=?, LAUNCHTYPE=? WHERE DEPLOYMENTID=? ";
