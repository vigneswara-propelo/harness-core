...
public class EcsSyncEventWriterIntegrationTest extends CategoryTest implements EcsEventGenerator {
  ...
  public void setUpData() throws Exception {
    ...
    PublishedMessage ec2ContainerInstanceInfoMessage = getContainerInstanceInfoMessage(
        TEST_INSTANCE_ID_STOPPED, TEST_INSTANCE_ID, TEST_CLOUD_PROVIDER_ID, TEST_CLUSTER_ARN, TEST_ACCOUNT_ID);
    ...
    PublishedMessage runningEc2InstanceInfoMessage = getContainerInstanceInfoMessage(
        TEST_INSTANCE_ID_RUNNING, TEST_INSTANCE_ID, TEST_CLOUD_PROVIDER_ID, TEST_CLUSTER_ARN, TEST_ACCOUNT_ID);
    ...
    PublishedMessage runningEc2InstanceLifecycleStartMessage = getContainerInstanceLifecycleMessage(
        INSTANCE_START_TIMESTAMP, EVENT_TYPE_START, TEST_INSTANCE_ID_RUNNING, TEST_ACCOUNT_ID);
  ...
  public void shouldStopEc2Instance() throws Exception {
    PublishedMessage ec2InstanceInfoMessage = getContainerInstanceInfoMessage(
        TEST_ACTIVE_INSTANCE_ID, TEST_INSTANCE_ID, TEST_CLOUD_PROVIDER_ID, TEST_CLUSTER_ARN, TEST_ACCOUNT_ID);
    ecsContainerInstanceInfoWriter.write(getMessageList(ec2InstanceInfoMessage));

    PublishedMessage ec2InstanceLifecycleStartMessage = getContainerInstanceLifecycleMessage(
        INSTANCE_START_TIMESTAMP, EVENT_TYPE_START, TEST_INSTANCE_ID, TEST_ACCOUNT_ID);
    ecsContainerInstanceLifecycleWriter.write(getMessageList(ec2InstanceLifecycleStartMessage));
    ...
    PublishedMessage ecsSyncEventMessage =
        getEcsSyncEventMessage(TEST_ACCOUNT_ID, TEST_CLOUD_PROVIDER_ID, TEST_CLUSTER_ARN, Collections.emptyList(),
            activeEc2InstanceList, Collections.emptyList(), INSTANCE_STOP_TIMESTAMP);
    ...
    InstanceData stoppedInstanceData =
        instanceDataService.fetchActiveInstanceData(TEST_ACCOUNT_ID, TEST_INSTANCE_ID_RUNNING, stoppedInstanceState);
