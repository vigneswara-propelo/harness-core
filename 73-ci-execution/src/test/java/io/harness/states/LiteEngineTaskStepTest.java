...
public class LiteEngineTaskStepTest extends CIExecutionTest {
  ...
  public void shouldExecuteSync() {
    ...
    when(buildSetupUtils.executeCILiteEngineTask(eq(liteEngineTaskStepInfo), eq(ambiance)))
        .thenReturn(K8sTaskExecutionResponse.builder().commandExecutionStatus(CommandExecutionStatus.SUCCESS).build());
  ...
  @Owner(developers = ALEKSANDAR)
  @Category(UnitTests.class)
  public void shouldReturnFailedOnFailedExecution() {
    TaskExecutor<HDelegateTask> executor = mock(TaskExecutor.class);
    when(taskExecutorMap.get(TaskMode.DELEGATE_TASK_V3.name())).thenReturn(executor);
    when(executor.queueTask(eq(ambiance.getSetupAbstractions()), any())).thenReturn("taskId");

    when(buildSetupUtils.executeCILiteEngineTask(eq(liteEngineTaskStepInfo), eq(ambiance)))
        .thenReturn(K8sTaskExecutionResponse.builder().commandExecutionStatus(CommandExecutionStatus.FAILURE).build());
  ...
  @Owner(developers = ALEKSANDAR)
  @Category(UnitTests.class)
  public void shouldReturnFailedOnUnsupportedStepType() {
    LiteEngineTaskStepInfo liteEngineTaskStepInfo =
        LiteEngineTaskStepInfo.builder()
  ...
  public void shouldReturnErrorOnException() {
    ...
    when(buildSetupUtils.executeCILiteEngineTask(eq(liteEngineTaskStepInfo), eq(ambiance)))
        .thenThrow(new RuntimeException("Exception happened"));
