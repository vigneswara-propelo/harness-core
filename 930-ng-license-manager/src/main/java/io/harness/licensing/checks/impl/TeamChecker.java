...
public class TeamChecker implements LicenseEditionChecker {
  @Override
  public void preCheck(ModuleLicense moduleLicense, List<ModuleLicense> currentLicenses, EditionAction targetAction) {
    // TODO: verify not exceed subscribe limit in subscription case
  }
  ...
  public Set<EditionAction> getEditionActions(ModuleLicenseState currentModuleState, ModuleType moduleType,
    ...
    boolean paidEditionInOtherModules =
        ModuleLicenseHelper.isPaidEditionInOtherModules(moduleType, Edition.ENTERPRISE, lastExpiredActiveLicenseMap);
    if (paidEditionInOtherModules) {
      result.add(EditionAction.DISABLED_BY_ENTERPRISE);
      return result;
    }
