...
@Slf4j
@OwnedBy(PL)
public class BitbucketServerListFilesScmApiErrorHandler implements ScmApiErrorHandler {
  ...
  public void handleError(int statusCode, String errorMessage, ErrorMetadata errorMetadata) throws WingsException {
    switch (statusCode) {
      ...
      default:
        log.error(String.format("Error while listing bitbucket(server) files: [%s: %s]", statusCode, errorMessage));
