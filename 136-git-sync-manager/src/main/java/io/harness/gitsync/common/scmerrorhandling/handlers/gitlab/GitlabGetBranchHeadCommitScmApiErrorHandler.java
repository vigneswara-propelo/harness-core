...
import static io.harness.gitsync.common.scmerrorhandling.handlers.gitlab.ScmErrorExplanations.BRANCH_NOT_FOUND;
...
@Slf4j
@OwnedBy(PIPELINE)
public class GitlabGetBranchHeadCommitScmApiErrorHandler implements ScmApiErrorHandler {
  public static final String GET_BRANCH_HEAD_COMMIT_FAILED_MESSAGE =
      "Failed to fetch branch head commit details from Gitlab.";
      ...
      case 401:
        throw NestedExceptionUtils.hintWithExplanationException(
            ...
            ErrorMessageFormatter.formatMessage(
                GET_BRANCH_HEAD_COMMIT_FAILED_MESSAGE + INVALID_CONNECTOR_CREDS, errorMetadata),
            new ScmUnauthorizedException(errorMessage));
      case 404:
        throw NestedExceptionUtils.hintWithExplanationException(
            ErrorMessageFormatter.formatMessage(REPO_NOT_FOUND, errorMetadata),
            ErrorMessageFormatter.formatMessage(GET_BRANCH_HEAD_COMMIT_FAILED_MESSAGE + REPO_NOT_FOUND, errorMetadata),
            new ScmBadRequestException(errorMessage));
      case 422: // TODO: check
        throw NestedExceptionUtils.hintWithExplanationException(
            ErrorMessageFormatter.formatMessage(BRANCH_NOT_FOUND, errorMetadata),
            ErrorMessageFormatter.formatMessage(
                GET_BRANCH_HEAD_COMMIT_FAILED_MESSAGE + BRANCH_NOT_FOUND, errorMetadata),
            new ScmBadRequestException(SCMExceptionErrorMessages.BRANCH_NOT_FOUND_ERROR));
      default:
        log.error(String.format(
            "Error while fetching the branch head commit from gitlab: [%s: %s]", statusCode, errorMessage));
