...
@Slf4j
@OwnedBy(PL)
public class UpdateTheRepoProvidersInGitSyncErrors implements NGMigration {
  private final MongoTemplate mongoTemplate;

  @Override
  public void migrate() {
    try {
      log.info("Started the migration to add the repo providers in git sync errors");
  ...
  private void updateTheTypeBitbucketInTheRecords() {
    ...
    Criteria criteriaForErrorsInBitbucket = Criteria.where(GitSyncErrorKeys.repoUrl).regex("/.*bitbucket.*/");
  ...
  private void updateTheTypeGithubInTheRecords() {
    ...
    Criteria criteriaForErrorsInGithub = Criteria.where(GitSyncErrorKeys.repoUrl).not().regex("/.*bitbucket.*/");
