...
public class BitbucketCreateFileScmApiErrorHandlerTest extends GitSyncTestBase {
  ...
  public void testHandleErrorOnUnauthorizedResponse() {
    try {
      bitbucketCreateFileScmApiErrorHandler.handleError(401, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
  ...
  public void testHandleErrorOnUnauthenticatedResponse() {
    try {
      bitbucketCreateFileScmApiErrorHandler.handleError(403, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
      WingsException exception = ExceptionUtils.cause(ScmUnauthorizedException.class, ex);
      assertThat(exception).isNotNull();
      assertThat(exception.getMessage()).isEqualTo(errorMessage);
  ...
  public void testHandleErrorOnResourceNotFoundResponse() {
    try {
      bitbucketCreateFileScmApiErrorHandler.handleError(404, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
  ...
  public void testHandleErrorWhenUnexpectedStatusCode() {
    try {
      bitbucketCreateFileScmApiErrorHandler.handleError(405, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
  ...
  public void testHandleErrorWhenRateLimitCode() {
    try {
      bitbucketCreateFileScmApiErrorHandler.handleError(429, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
      WingsException exception = ExceptionUtils.cause(ScmBadRequestException.class, ex);
      assertThat(exception).isNotNull();
      assertThat(exception.getMessage()).isEqualTo(errorMessage);
