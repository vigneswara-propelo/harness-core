...
public class BitbucketListFilesScmApiErrorHandlerTest extends GitSyncTestBase {
  ...
  public void testHandleErrorWhenRateLimitCode() {
    try {
      bitbucketListFilesScmApiErrorHandler.handleError(429, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
      WingsException exception = ExceptionUtils.cause(ScmBadRequestException.class, ex);
      assertThat(exception).isNotNull();
      assertThat(exception.getMessage()).isEqualTo(errorMessage);
