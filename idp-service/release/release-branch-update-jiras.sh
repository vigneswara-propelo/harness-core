...
KEYS=$(git log --pretty=oneline --abbrev-commit |\
      ...
      grep -o -iE '('IDP')-[0-9]+' | sort | uniq)

echo $KEYS

#FIX_IDP_VERSION value to be same as used in release-branch-create-idp-version.sh
FIX_IDP_VERSION="IDP-""$VERSION"
  ...
  do
    ...
    if [[ $EXCLUDE_PROJECTS == *",$PROJ,"* ]]; then
      ...
      if [[ "$response" -eq 204 ]] ; then
        echo "$KEY fixVersion set to $FIX_IDP_VERSION"
