...
public class GithubMeanTimeToCompleteWorkflowRunsDsl implements DataSourceLocation {
  ...
  public Map<String, Object> fetchData(String accountIdentifier, BackstageCatalogEntity backstageCatalogEntity,
    ...
    Optional<Map.Entry<DataPointEntity, Set<String>>> dataPointAndInputValuesOpt =
        dataPointsAndInputValues.entrySet()
            .stream()
            .filter(entry -> entry.getKey().getIdentifier().equals(MEAN_TIME_TO_COMPLETE_WORKFLOW_RUNS))
            .findFirst();
  ...
  public String replaceInputValuePlaceholdersIfAny(Map<String, String> dataPointIdsAndInputValue, String url) {
    if (!isEmpty(dataPointIdsAndInputValue.get(MEAN_TIME_TO_COMPLETE_WORKFLOW_RUNS))) {
      String inputValue = dataPointIdsAndInputValue.get(MEAN_TIME_TO_COMPLETE_WORKFLOW_RUNS);
