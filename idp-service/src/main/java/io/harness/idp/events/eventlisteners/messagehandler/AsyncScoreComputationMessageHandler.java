...
public class AsyncScoreComputationMessageHandler implements EventMessageHandler {
  ...
  public void handleMessage(Message message, EntityChangeDTO entityChangeDTO, String action) {
    ...
    if (action.equals(CREATE_ACTION)) {
      asyncScoreComputationService.triggerScoreComputation(entityChangeDTO);
    } else {
      log.warn("ACTION - {} is not to be handled by IDP async score computation event handler", action);
    }
  }
}
