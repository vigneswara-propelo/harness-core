...
import io.harness.idp.pipeline.filter.IDPFilter;
...
public class IDPStageFilterCreator extends GenericStageFilterJsonCreatorV2<IDPStageNode> {
  ...
  public PipelineFilter getFilter(FilterCreationContext filterCreationContext, IDPStageNode stageNode) {
    ...
    IDPFilter.IDPFilterBuilder idpFilterBuilder = IDPFilter.builder();
    ...
    if (ciCodeBase != null) {
      if (ciCodeBase.getRepoName().getValue() != null) {
        idpFilterBuilder.repoName(ciCodeBase.getRepoName().getValue());
        ...
        try {
          ...
          idpFilterBuilder.repoName(repoName);
