...
export PURPOSE=queue-service/hsqs
...
if [ ! -z "$NOT_MERGED" ]
then
    echo "ERROR: There are jira issues in debezium-service release branches that are not reflected in develop."
    exit 1
fi
...
if [[ "$EXECUTE_NEW_CODE" == "true" ]]; then
    ...
    export VERSION_FILE=debezium-service/build.properties
