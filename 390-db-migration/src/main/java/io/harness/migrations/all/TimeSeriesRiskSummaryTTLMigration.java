...
public class TimeSeriesRiskSummaryTTLMigration implements Migration {
  ...
  private void deleteOldRecords(Instant deletionThreshold) {
    try {
      Query<TimeSeriesRiskSummary> query = wingsPersistence.createQuery(TimeSeriesRiskSummary.class, excludeAuthority)
      ...
      Query<TimeSeriesRiskSummary> limitedQuery = query.limit(BATCH_SIZE);
