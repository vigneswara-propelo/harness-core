...
export FIX_ACL_VERSION="AccessControl_""$VERSION"00
RELDATE=$(date +'%Y-%m-%d')
PROJ="PL"
...
if  [[ "$response" -ne 201 ]] ; then
  echo "Failed to create version $FIX_ACL_VERSION in $PROJ - Response code: $response"
  if [[ "$response" -eq 404 ]] ; then
    echo "404 response indicates that user $JIRA_USERNAME does not have permissions to create versions in project $PROJ"
  fi
else
  echo "Successfully created version $FIX_ACL_VERSION in $PROJ"
fi

# make sure fix version is available
FIX_ACL_VERSION_ID=$(curl -X GET -H "Content-Type: application/json" \
      https://harness.atlassian.net/rest/api/2/project/PL/versions \
      --user $JIRA_USERNAME:$JIRA_PASSWORD \
      | jq -c '.[] | select( .name == "'$FIX_ACL_VERSION'" )' | jq '.id' | tr -d '"')

if [[ -z "$FIX_ACL_VERSION_ID" ]]; then
