...
public class RecastReflectionUtils {
  ...
  public static boolean isPropertyType(final Class type) {
    return type != null && (isPrimitiveLike(type));
  }
  ...
  public static boolean shouldSaveClassName(final Object rawVal, final Object convertedVal, final CastedField cf) {
    if (rawVal == null || cf == null) {
      return true;
    }
    if (cf.isSingleValue()) {
      return !(cf.getType().equals(rawVal.getClass()) && !(convertedVal instanceof BasicDBList));
    }
    boolean isDBObject = convertedVal instanceof DBObject;
    boolean anInterface = cf.getSubClass().isInterface();
