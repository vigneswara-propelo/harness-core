...
def test_default_tokenizer():
    ...
    tokens = Tokenizer.default_tokenizer(str)
    expected_tokens = ['info', 'org.quartz.plugins.history.loggingtriggerhistoryplugin', 'trigger', 'default.vj3t7b6htyyugnj0jifqxa', 'completed', 'firing', 'job', 'vj3t7b6htyyugnj0jifqxa.artifact_stream_cron_group', 'at', 'with', 'resulting', 'trigger', 'instruction', 'code', 'do', 'nothing']
...
def test_single_token():
    str = '''172.31.15.70 - - [31/May/2017:21:35:12 +0000] \"/todolist/health-check HTTP/1.1\" 200 50'''
    tokens = Tokenizer.default_tokenizer(str)
    assert len(tokens) == 1
    assert tokens[0] == str