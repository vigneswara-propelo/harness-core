...
@UtilityClass
public class DelegateTokenUtils {
  /**
   ...
   */
  public static String getDecodedTokenString(String delegateToken) {
    // Step 1. Check if the token is base64 encoded
    if (!Base64.isBase64(delegateToken)) {
      return delegateToken;
    }
    // Step 2. decode the token, check if the decoded is hex decimal string
    String decoded = new String(Base64.decodeBase64(delegateToken)).trim();
