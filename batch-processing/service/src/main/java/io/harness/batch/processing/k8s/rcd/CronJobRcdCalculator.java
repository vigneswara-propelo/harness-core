...
public class CronJobRcdCalculator implements ResourceClaimDiffCalculator {
  ...
  public ResourceClaimDiff computeResourceClaimDiff(String oldYaml, String newYaml) {
    V1PodSpec oldPodSpec = Optional.ofNullable(Yaml.loadAs(oldYaml, V1CronJob.class))
                               ...
                               .orElse(null);
    V1PodSpec newPodSpec = Optional.ofNullable(Yaml.loadAs(newYaml, V1CronJob.class))
                               .map(V1CronJob::getSpec)
                               .map(V1CronJobSpec::getJobTemplate)
                               .map(V1JobTemplateSpec::getSpec)
                               .map(V1JobSpec::getTemplate)
