...
public class InspectCommand<T extends io.dropwizard.Configuration> extends ConfiguredCommand<T> {
  ...
  protected void run(Bootstrap<T> bootstrap, Namespace namespace, T configuration) {
    ...
    mainConfiguration.setHarnessSTOMongo(
        mainConfiguration.getHarnessSTOMongo().toBuilder().indexManagerMode(IndexManager.Mode.INSPECT).build());
    ...
    modules.add(new AbstractModule() {
      ...
      MongoConfig mongoConfig() {
        return mainConfiguration.getHarnessSTOMongo();
      }
    });
