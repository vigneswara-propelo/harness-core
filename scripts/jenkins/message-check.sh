...
PR_MESSAGE=`echo "${ghprbPullTitle}" | grep -iE '\[(CCM|CCE|CD|CE|CV|DOC|ER|HAR|LE|PL|SEC|SWAT)-[0-9]+]:'`

if [ -z "$PR_MESSAGE" ]
then
    echo The PR title \"${ghprbPullTitle}\"
    echo "does not match the expectations"
    echo 'Make sure that your message starts with [CCM|CCE|CD|CE|CV|DOC|ER|HAR|LE|PL|SEC|SWAT-<number>]: <description>'
    exit 1
fi

KEY=`echo "${ghprbPullTitle}" | grep -o -iqE '(CCM|CCE|CD|CE|CV|DOC|ER|HAR|LE|PL|SEC|SWAT)-[0-9]+'`
