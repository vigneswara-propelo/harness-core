...
git log --remotes=origin/release/* --pretty=oneline --abbrev-commit | grep -iE '\[(HAR|CD|PL|LE)-[0-9]+]:' -o | sort | uniq > release.txt
git log --remotes=origin/[m]aster --pretty=oneline --abbrev-commit | grep -iE '\[(HAR|CD|PL|LE)-[0-9]+]:' -o | sort | uniq > master.txt
