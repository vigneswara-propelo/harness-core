KEYS=`git log --pretty=oneline --abbrev-commit | awk "/${PREVIOUS_CUT_COMMIT_MESSAGE}/ {exit} {print}" | grep -o -iE '(CCM|CCE|CD|CE|CV|DOC|ER|HAR|LE|PL|SEC|SWAT)-[0-9]+' | sort | uniq`
