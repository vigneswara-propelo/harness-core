...
@Slf4j
public class AwsCdkDestroyPluginInfoProviderTest extends CategoryTest {
  @Rule public MockitoRule mockitoRule = MockitoJUnit.rule();
  @InjectMocks @Spy private AwsCdkDestroyPluginInfoProvider awsCdkDestroyPluginInfoProvider;
  ...
  public void testGetPluginInfo() {
    ...
    doReturn(cdAbstractStepNode).when(awsCdkDestroyPluginInfoProvider).getCdAbstractStepNode(any(), any());
    AwsCdkDestroyStepInfo awsCdkDestroyStepInfo =
        AwsCdkDestroyStepInfo.infoBuilder()
            ...
            .build();
    doReturn(awsCdkDestroyStepInfo).when(cdAbstractStepNode).getStepSpecType();

    PluginCreationResponseWrapper pluginCreationResponseWrapper =
        awsCdkDestroyPluginInfoProvider.getPluginInfo(pluginCreationRequest, new HashSet<>(), ambiance);
