...
public class K8sCanaryStepInfo extends K8sCanaryStepParameters implements CDStepInfo, Visitable {
  ...
  public K8sCanaryStepInfo(InstanceSelectionWrapper instanceSelection, ParameterField<String> timeout,
      ParameterField<Boolean> skipDryRun, Map<String, StepDependencySpec> stepDependencySpecs, String name,
      String identifier) {
    super(instanceSelection, timeout, skipDryRun, stepDependencySpecs);
    this.name = name;
  ...
  public void registerStepDependencyInstructors(
    ...
    OutcomeRefStepDependencyInstructor instructor =
        OutcomeRefStepDependencyInstructor.builder()
            .key(ParentPathInfoUtils.getParentPath(context) + "." + CDStepDependencyKey.K8S_CANARY.name())
            .providerPlanNodeId(nodeId)
            .outcomeExpression(OutcomeExpressionConstants.K8S_CANARY_OUTCOME)
            .build();
