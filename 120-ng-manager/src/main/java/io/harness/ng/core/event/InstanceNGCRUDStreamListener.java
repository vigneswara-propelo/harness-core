...
@Slf4j
@Singleton
public class InstanceNGCRUDStreamListener implements MessageListener {
  private final InstanceService instanceService;
  @Inject
  public InstanceNGCRUDStreamListener(InstanceService instanceService) {
  ...
  public boolean handleMessage(Message message) {
    ...
    if (metadataMap != null && PROJECT_ENTITY.equals(metadataMap.get(ENTITY_TYPE))) {
  ...
  private boolean processProjectDeleteEvent(ProjectEntityChangeDTO projectEntityChangeDTO) {
    return instanceService.deleteForProject(projectEntityChangeDTO.getAccountIdentifier(),
