...
public class DeploymentStageConfigResource {
  ...
  getCdDeployStageMetadata(@Parameter(description = "Stage yaml string to be parsed") @NotNull String cdStageYaml)
      throws IOException {
    CDStageMetaDataDTOBuilder CDStageMetaDataDTOBuilder = CDStageMetaDataDTO.builder();
    final DeploymentStageNode deploymentStageNode = YamlUtils.read(cdStageYaml, DeploymentStageNode.class);
    final DeploymentStageConfig deploymentStageConfig = deploymentStageNode.getDeploymentStageConfig();

    final String serviceRef = getServiceRef(deploymentStageConfig);
    if (isNotBlank(serviceRef)) {
      CDStageMetaDataDTOBuilder.serviceRef(serviceRef);
    }
    final String environmentRef = getEnvironmentRef(deploymentStageConfig);
