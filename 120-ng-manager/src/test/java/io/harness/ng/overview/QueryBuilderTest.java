...
public class QueryBuilderTest extends CategoryTest {
  ...
  public void testQueryBuilderServiceDeployments() {
    String expectedQueryResult =
        "select status, time_entity, COUNT(*) as numberOfRecords from (select service_status as status, service_startts as execution_time, harness_date_bin_ng_mgr(86400000, service_startts) as time_entity, pipeline_execution_summary_cd_id  from service_infra_info as sii, pipeline_execution_summary_cd as pesi where pesi.accountid = sii.accountid AND sii.service_id is not null and pesi.accountid='account' and pesi.orgidentifier='org' and pesi.projectidentifier='project' and sii.service_id='service_id' and pesi.id=sii.pipeline_execution_summary_cd_id and sii.service_startts>=1620000000000 and sii.service_startts<1620950400000) as service where status != '' group by status, time_entity;";
