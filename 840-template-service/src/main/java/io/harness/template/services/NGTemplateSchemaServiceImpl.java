...
public class NGTemplateSchemaServiceImpl implements NGTemplateSchemaService {
    ...
    @Inject
    public NGTemplateSchemaServiceImpl(PipelineYamlSchemaServiceClient pipelineYamlSchemaServiceClient, YamlSchemaProvider yamlSchemaProvider,
    ...
    private JsonNode getTemplateYamlSchemaInternal(String accountIdentifier, String projectIdentifier, String orgIdentifier, Scope scope, String templateChildType, TemplateEntityType templateEntityType) {
        ...
        JsonNode specSchema = NGRestUtils
                .getResponse(pipelineYamlSchemaServiceClient.getYamlSchema(accountIdentifier, orgIdentifier, projectIdentifier, yamlGroup, entityType, scope)).getSchema();
    ...
    public void validateYamlSchemaInternal(TemplateEntity templateEntity) {
        ...
        if(!TemplateYamlSchemaMergeHelper.isFeatureFlagEnabled(FeatureName.TEMPLATE_SCHEMA_VALIDATION, accountIdentifier, accountClient)){
            return;
        }
        try {
            ...
            if(templateEntity.getTemplateEntityType().equals(TemplateEntityType.PIPELINE_TEMPLATE)) {
                ...
                yamlSchemaValidator.validate(templateYaml, schemaString,
                        TemplateYamlSchemaMergeHelper.isFeatureFlagEnabled(FeatureName.DONT_RESTRICT_PARALLEL_STAGE_COUNT, accountIdentifier, accountClient),
                        allowedParallelStages, pathToJsonNode);
            }else{
                yamlSchemaValidator.validate(templateYaml, schemaString, true,
