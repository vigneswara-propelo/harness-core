    ...
    private static Set<String> getKeysToRemoveFromTemplateSpec(TemplateEntityType templateEntityType){
        switch (templateEntityType){
            ...
            case PIPELINE_TEMPLATE:
                return new HashSet<>(Arrays.asList(NAME, IDENTIFIER, DESCRIPTION, TYPE, TAGS, ORG_IDENTIFIER, PROJECT_IDENTIFIER, TEMPLATE));
            default:
                return new HashSet<>();
        }
    }

    public static boolean isFeatureFlagEnabled(FeatureName featureName, String accountIdentifier, AccountClient accountClient) {
        return RestClientUtils.getResponse(
                accountClient.isFeatureFlagEnabled(featureName.name(), accountIdentifier));
    }
}
