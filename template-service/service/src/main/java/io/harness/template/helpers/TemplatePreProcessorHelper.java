...
public class TemplatePreProcessorHelper {
  ...
  public Map<String, Object> preProcessResMap(
      TemplateEntity templateEntity, Map<String, Object> resMap, Set<String> idsValuesSet) {
    ...
    if (templateEntity.getTemplateEntityType() == TemplateEntityType.PIPELINE_TEMPLATE) {
      ...
      if (HarnessYamlVersion.isV1(templateEntity.getHarnessVersion())) {
        resMap = preProcessResMapInternal(resMap, HarnessYamlVersion.V1, idsValuesSet, yamlPreProcessor);
        return resMap;
      } else {
        resMap = preProcessResMapInternal(resMap, HarnessYamlVersion.V0, idsValuesSet, yamlPreProcessor);
        return resMap;
      }
    }
  }

  private Map<String, Object> preProcessResMapInternal(
      Map<String, Object> resMap, String version, Set<String> idsValuesSet, YamlPreProcessor yamlPreprocessor) {
    injectIdInYaml(version, resMap, idsValuesSet, yamlPreprocessor);
    switch (version) {
  ...
  private void injectIdInYaml(
      String version, Map<String, Object> resMap, Set<String> idsValuesSet, YamlPreProcessor yamlPreprocessor) {
    ...
    if (resMap.get(idField) == null) {
      ...
      String idWithSuffix = yamlPreprocessor.getIdWithSuffix(idsValuesSet, id);
