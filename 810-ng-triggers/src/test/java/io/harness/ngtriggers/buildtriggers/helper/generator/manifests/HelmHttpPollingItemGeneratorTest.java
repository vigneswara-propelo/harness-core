...
public class HelmHttpPollingItemGeneratorTest extends CategoryTest {
  ...
  String helm_http_pipeline_manifest_snippet_runtime_tagonly;
  String ng_trigger_manifest_helm_http;
  ...
  public void setUp() throws IOException {
    ...
    helm_http_pipeline_manifest_snippet_runtime_all = Resources.toString(
        Objects.requireNonNull(classLoader.getResource("helm_http_pipeline_manifest_snippet_runtime_all.yaml")),
        StandardCharsets.UTF_8);
    helm_http_pipeline_manifest_snippet_runtime_tagonly = Resources.toString(
        Objects.requireNonNull(classLoader.getResource("helm_http_pipeline_manifest_snippet_runtime_tagonly.yaml")),
        StandardCharsets.UTF_8);

    ng_trigger_manifest_helm_http = Resources.toString(
  ...
  public void testHelmS3PollingItemGeneration_pipelineContainsFixedValuesExceptTag() throws Exception {
    TriggerDetails triggerDetails =
        ngTriggerElementMapper.toTriggerDetails("acc", "org", "proj", ng_trigger_manifest_helm_http);
    ...
    BuildTriggerOpsData buildTriggerOpsData = buildTriggerHelper.generateBuildTriggerOpsDataForManifest(
        triggerDetails, helm_http_pipeline_manifest_snippet_runtime_tagonly);
    PollingItem pollingItem = httpHelmPollingItemGenerator.generatePollingItem(buildTriggerOpsData);
    ...
    assertThat(pollingItem.getPollingPayloadData().getHttpHelmPayload()).isNotNull();
    assertThat(pollingItem.getPollingPayloadData().getHttpHelmPayload().getChartName()).isEqualTo("chart");
    assertThat(pollingItem.getPollingPayloadData().getHttpHelmPayload().getHelmVersion().name()).isEqualTo("V2");
  ...
  @Owner(developers = ADWAIT)
  @Category(UnitTests.class)
  public void testHelmHttpPollingItemGeneration_pipelineContainsAllRuntimeInputs() throws Exception {
    TriggerDetails triggerDetails =
        ngTriggerElementMapper.toTriggerDetails("acc", "org", "proj", ng_trigger_manifest_helm_http);
    triggerDetails.getNgTriggerEntity().getMetadata().getBuildMetadata().setPollingConfig(
        PollingConfig.builder().signature("sig1").build());
    BuildTriggerOpsData buildTriggerOpsData = buildTriggerHelper.generateBuildTriggerOpsDataForManifest(
        triggerDetails, helm_http_pipeline_manifest_snippet_runtime_all);
    PollingItem pollingItem = httpHelmPollingItemGenerator.generatePollingItem(buildTriggerOpsData);
    ...
    assertThat(pollingItem.getPollingPayloadData().getHttpHelmPayload().getHelmVersion().name()).isEqualTo("V3");
