...
type Db interface {
	...
	Write(ctx context.Context, table, accountID, orgId, projectId, buildId, stageId, stepId,
		report string, tests ...*types.TestCase) error

	// Summary provides a high-level test case summary
	Summary(ctx context.Context, table, accountID, orgId, projectId, buildId, report string) (types.SummaryResponse, error)

	// GetTestCases returns test cases corresponding to a specific suite
	GetTestCases(ctx context.Context, table, accountID, orgId, projectId, buildId,
	...
	GetTestSuites(ctx context.Context, table, accountID, orgId, projectId, buildId,
