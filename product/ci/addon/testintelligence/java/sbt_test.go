...
func TestSBT_GetCmd(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:                 "sbt -Duser.timezone=US/Mountain -Duser.locale=en/US -J-javaagent:/addon/bin/java-agent.jar=/test/tmp/config.ini 'test'",
			expectedErr:          false,
		...
		},
		{
			...
			args:                 "clean test",
			runOnlySelectedTests: false,
			want:                 "sbt clean test -J-javaagent:/addon/bin/java-agent.jar=/test/tmp/config.ini 'test'",
			expectedErr:          false,
			tests:                []types.RunnableTest{},
		},
		{
			...
			want:                 "sbt clean test -Duser.timezone=US/Mountain -Duser.locale=en/US -J-javaagent:/addon/bin/java-agent.jar=/test/tmp/config.ini 'testOnly pkg1.cls1 pkg2.cls2'",
			expectedErr:          false,
			tests:                []types.RunnableTest{t1, t2},
		},
		{
		...
		},
		{
			...
			runOnlySelectedTests: true,
			want:                 "sbt clean test -B -2C-Duser.timezone=US/Mountain -Duser.locale=en/US -J-javaagent:/addon/bin/java-agent.jar=/test/tmp/config.ini 'testOnly pkg1.cls1 pkg2.cls2'",
			expectedErr:          false,
		...
		},
		{
			...
			runOnlySelectedTests: true,
			want:                 "sbt clean test -B -2C -Duser.timezone=US/Mountain -Duser.locale=en/US || true -J-javaagent:/addon/bin/java-agent.jar=/test/tmp/config.ini 'testOnly pkg2.cls2'",
			expectedErr:          false,
