...
export FIX_CIE_VERSION="CIE-""$VERSION"
RELDATE=$(date +'%Y-%m-%d')
...
if  [[ "$response" -ne 201 ]] ; then
  echo "Failed to create version $FIX_CIE_VERSION in $PROJ - Response code: $response"
  if [[ "$response" -eq 404 ]] ; then
    echo "404 response indicates that user $JIRA_USERNAME does not have permissions to create versions in project $PROJ"
  fi
else
  echo "Successfully created version $FIX_CIE_VERSION in $PROJ"
fi
