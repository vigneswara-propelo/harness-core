...
@Slf4j
@Singleton
public class AccountEventConsumer implements Runnable {
  ...
  @Override
  public void run() {
    log.info("Started the consumer for entity_crud");
    try {
      SecurityContextBuilder.setContext(new ServicePrincipal(CI_MANAGER.getServiceId()));
      while (!Thread.currentThread().isInterrupted()) {
        if (queueController.isNotPrimary()) {
          log.info("entity_crud consumer is not running on primary deployment, will try again after some time...");
      ...
      log.error("entity_crud consumer unexpectedly stopped", ex);
    } finally {
      SecurityContextBuilder.unsetCompleteContext();
    }
  }
