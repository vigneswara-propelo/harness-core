...
public class CIModuleInfoMapper {
  ...
  private static CIBuildPRHook convertPR(PRWebhookEvent pr) {
  ...
  private static CIBuildBranchHook convertBranch(BranchWebhookEvent branch) {
    List<CIBuildCommit> commitList = new ArrayList<>();
  ...
  private static CIBuildCommit convertCommit(CommitDetails commit) {
  ...
  private static long convertToMilliSeconds(long seconds) {
    return seconds * 1000;
  }
}
