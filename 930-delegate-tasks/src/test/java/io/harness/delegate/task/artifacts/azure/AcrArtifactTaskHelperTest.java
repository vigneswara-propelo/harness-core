...
public class AcrArtifactTaskHelperTest extends CategoryTest {
  ...
  @Mock private AcrArtifactTaskHandler acrArtifactTaskHandler;

  @InjectMocks private AcrArtifactTaskHelper acrArtifactTaskHelper;
  ...
  public void testGetAcrArtifactTaskHelperGetBuildSuccess() {
    ...
    doNothing().when(acrArtifactTaskHandler).decryptRequestDTOs(acrArtifactDelegateRequest);

    List<AcrArtifactDelegateResponse> responseList = new LinkedList<>();
    responseList.add(createAcrArtifactDelegateResponse("1.0"));
    responseList.add(createAcrArtifactDelegateResponse("2.0"));
    ...
    when(acrArtifactTaskHandler.getBuilds(acrArtifactDelegateRequest)).thenReturn(artifactTaskExecutionResponse);

    ArtifactTaskResponse artifactTaskResponse =
        acrArtifactTaskHelper.getArtifactCollectResponse(artifactTaskParameters);

    assertThat(artifactTaskResponse.getCommandExecutionStatus()).isEqualTo(CommandExecutionStatus.SUCCESS);
    List<ArtifactDelegateResponse> response =
        artifactTaskResponse.getArtifactTaskExecutionResponse().getArtifactDelegateResponses();

    assertThat(response.size()).isEqualTo(2);
    assertThat(response.get(0).getBuildDetails().getMetadata().get(ArtifactMetadataKeys.TAG)).isEqualTo("1.0");
    assertThat(response.get(1).getBuildDetails().getMetadata().get(ArtifactMetadataKeys.TAG)).isEqualTo("2.0");
  }

  @Test
  @Owner(developers = MLUKIC)
  @Category(UnitTests.class)
  public void testGetAcrArtifactTaskHelperGetLastSuccessfulBuildSuccess() {
    AzureConnectorDTO azureConnectorDTO = getSPConnector(AzureSecretType.SECRET_KEY);
    List encryptedDataDetails = Lists.emptyList();
    ...
    when(acrArtifactTaskHandler.getLastSuccessfulBuild(acrArtifactDelegateRequest))
    ...
    assertThat(response.get(0).getBuildDetails().getMetadata().get(ArtifactMetadataKeys.TAG)).isEqualTo("2.0");
  }

  @Test
  @Owner(developers = MLUKIC)
  @Category(UnitTests.class)
  public void testGetAcrArtifactTaskHelperWrongtaskTypeFailure() {
    AcrArtifactDelegateRequest acrArtifactDelegateRequest = AcrArtifactDelegateRequest.builder().build();
