...
@OwnedBy(CDP)
public class AsgShiftTrafficCommandTaskHandlerTest extends CategoryTest {
  ...
  @Spy @InjectMocks private AsgShiftTrafficCommandTaskHandler taskHandler;
  ...
  @Before
  public void setup() {
    doReturn(logCallback).when(asgTaskHelper).getLogCallback(any(), anyString(), anyBoolean(), any());
  ...
  public void executeTaskInternalTest() {
    ...
    AsgShiftTrafficRequest request = createRequest(lbConfig, false, 10);

    AsgShiftTrafficResponse response = (AsgShiftTrafficResponse) taskHandler.executeTaskInternal(
        request, iLogStreamingTaskClient, commandUnitsProgress);

    assertThat(response.getCommandExecutionStatus()).isEqualTo(CommandExecutionStatus.SUCCESS);

    AsgShiftTrafficResult result = response.getResult();
  ...
  public void executeTaskInternalWithDownsizeTest() {
    AsgLoadBalancerConfig lbConfig = getAsgLoadBalancerConfig();
    AsgShiftTrafficRequest request = createRequest(lbConfig, true, 10);
  ...
  private AsgShiftTrafficRequest createRequest(
      AsgLoadBalancerConfig asgLoadBalancerConfig, boolean downsizeOldAsg, int weight) {
    return AsgShiftTrafficRequest.builder()
