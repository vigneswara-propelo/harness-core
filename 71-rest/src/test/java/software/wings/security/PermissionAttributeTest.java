...
public class PermissionAttributeTest extends WingsBaseTest {
  ...
  public void testAddedPermissionsAreInSync() {
    assertThat(PermissionAttribute.PermissionType.values().length).isEqualTo(36);
    assertThat(QLAccountPermissionType.values().length).isEqualTo(24);
    assertThat(PermissionAttribute.PermissionType.values().length - 36)
        .isEqualTo(QLAccountPermissionType.values().length - 24);
  }
