...
public class ServiceElasticsearchRequestHandlerTest extends WingsBaseTest {
  @Mock @Named(AuditTrailFeature.FEATURE_NAME) PremiumFeature auditTrailFeature;
  @Inject @InjectMocks ServiceElasticsearchRequestHandler serviceSearchRequestHandler;
  ...
  public void translateHitsToSearchResultsTest() {
    ...
    SearchResponse searchResponse = SearchRequestHandlerTestUtils.getSearchResponse(ServiceSearchEntity.TYPE);
    when(auditTrailFeature.isAvailableForAccount(accountId)).thenReturn(true);

    List<SearchResult> searchResults =
        serviceSearchRequestHandler.translateHitsToSearchResults(searchResponse.getHits(), accountId);
    assertThat(searchResults).isNotNull();
    assertThat(searchResults.size()).isEqualTo(1);
    assertThat(searchResults.get(0).getType()).isEqualTo(EntityType.SERVICE);
  }
  ...
  private List<AppPermissionSummary> buildAppPermissionSummary() {
    ...
    Map<PermissionAttribute.Action, Set<String>> servicePermissions =
        new HashMap<PermissionAttribute.Action, Set<String>>() {
  ...
  private List<SearchResult> getSearchResult() {
    ServiceSearchResult searchResult1 = new ServiceSearchResult();
    ServiceSearchResult searchResult2 = new ServiceSearchResult();

    searchResult1.setAppId(appId1);
    searchResult1.setAccountId(accountId);
    searchResult1.setId(serviceId1);
    searchResult1.setType(EntityType.SERVICE);
    searchResult2.setAppId(appId2);
    searchResult2.setAccountId(accountId);
    searchResult2.setId(serviceId2);
    searchResult2.setType(EntityType.SERVICE);
  ...
  public void filterSearchResultsTest() {
    ...
    List<SearchResult> newSearchResults = serviceSearchRequestHandler.filterSearchResults(searchResults);

    assertThat(newSearchResults.size()).isEqualTo(1);

    assertThat(((ServiceSearchResult) newSearchResults.get(0))
                   .getWorkflows()
    ...
    assertThat(((ServiceSearchResult) newSearchResults.get(0)).getAudits()).isNotEmpty();
    assertThat(((ServiceSearchResult) newSearchResults.get(0)).getDeployments().size()).isEqualTo(1);
    ...
    newSearchResults = serviceSearchRequestHandler.filterSearchResults(newSearchResults);

    assertThat(((ServiceSearchResult) newSearchResults.get(0)).getAudits()).isEmpty();
  }
}
