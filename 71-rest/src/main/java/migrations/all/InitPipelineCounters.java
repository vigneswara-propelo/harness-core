...
public class InitPipelineCounters implements Migration {
  ...
  @Override
  public void migrate() {
    logger.info("Initializing Pipeline Counters");
    ...
    try {
      ...
      for (Account account : accounts) {
        ...
        logger.info("Initializing Counter. Account Id: {} , PipelineCount: {}", accountId, pipelineCount);
        Counter counter = new Counter(action.key(), pipelineCount);
        wingsPersistence.save(counter);
      }
    } catch (Exception e) {
      logger.error("Error initializing Pipeline counters", e);
    }
  }
}
