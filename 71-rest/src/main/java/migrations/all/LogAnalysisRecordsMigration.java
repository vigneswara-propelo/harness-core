...
public class LogAnalysisRecordsMigration implements Migration {
  private static final Logger logger = LoggerFactory.getLogger(LogAnalysisRecordsMigration.class);

  @Inject WingsPersistence wingsPersistence;

  @Override
  public void migrate() {
    DBCollection collection = wingsPersistence.getCollection(DEFAULT_STORE, ReadPref.NORMAL, "logAnalysisRecords");
    BulkWriteOperation bulkWriteOperation = collection.initializeUnorderedBulkOperation();

    DBCursor logAnalysisRecords =
        wingsPersistence.getCollection(DEFAULT_STORE, ReadPref.NORMAL, "logAnalysisRecords").find();
    ...
    while (logAnalysisRecords.hasNext()) {
      ...
      bulkWriteOperation
          .find(wingsPersistence.createQuery(LogMLAnalysisRecord.class).filter(ID_KEY, uuId).getQueryObject())
