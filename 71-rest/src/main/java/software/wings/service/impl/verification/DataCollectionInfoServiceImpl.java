  ...
  public DataCollectionInfoV2 create(CVConfiguration config, Instant startTime, Instant endTime) {
    ...
    switch (config.getStateType()) {
      case SPLUNKV2:
        ...
        dataCollectionInfo = SplunkDataCollectionInfoV2.builder()
      ...
      case NEW_RELIC:
        ...
        dataCollectionInfo = NewRelicDataCollectionInfoV2.builder()
        ...
        break;
      case ELK:
        ElkCVConfiguration elkCVConfiguration = (ElkCVConfiguration) config;
        ElkConfig elkConfig = (ElkConfig) settingsService.get(config.getConnectorId()).getValue();
        dataCollectionInfo = ElkDataCollectionInfoV2.builder()
                                 .elkConfig(elkConfig)
                                 .cvConfigId(config.getUuid())
                                 ...
                                 .stateExecutionId(CV_24x7_STATE_EXECUTION + "-" + config.getUuid())
                                 .serviceId(config.getServiceId())
                                 .query(elkCVConfiguration.getQuery())
                                 .indices(elkCVConfiguration.getIndex())
                                 .hostnameField(elkCVConfiguration.getHostnameField())
                                 .messageField(elkCVConfiguration.getMessageField())
                                 .timestampField(elkCVConfiguration.getTimestampField())
                                 .timestampFieldFormat(elkCVConfiguration.getTimestampFormat())
                                 .queryType(elkCVConfiguration.getQueryType())
                                 .startTime(startTime)
                                 .endTime(endTime)
                                 .build();
