...
import software.wings.graphql.schema.type.permissions.QLEnvFilterType;
...
public class UserGroupController {
  ...
  private GenericEntityFilter createGenericEntityFilter(QLAppFilter appFilter) {
  ...
  private AppPermission convertToAppPermissionEntity(QLAppPermissions permission) {
    ...
    Set<Action> actions = actionsList.stream().map(UserGroupController::mapAppActions).collect(Collectors.toSet());
    ...
    GenericEntityFilter appFilter = createGenericEntityFilter(permission.getApplications());
    Filter entityFilter;
    String filterType;
    switch (permissionType) {
      case ALL:
  ...
  public static Set<AppPermission> populateUserGroupAppPermissionEntity(
      QLSetUserGroupPermissionsParameters parameters) {
    List<QLAppPermissions> appPermissions = parameters.getPermissions().getAppPermissions();
    Set<AppPermission> userGroupAppPermissions = null;
    if (appPermissions != null) {
      userGroupAppPermissions =
          appPermissions.stream().map(UserGroupController::convertToAppPermissionEntity).collect(Collectors.toSet());
    }
    return userGroupAppPermissions;
  }

  // Populate the AccountPermission entity
  public static AccountPermissions populateUserGroupAccountPermissionEntity(
      QLSetUserGroupPermissionsParameters parameters) {
    Set<QLAccountPermissionType> accountPermissions = parameters.getPermissions().getAccountPermissions();
    if (accountPermissions == null) {
      return null;
    }
    Set<PermissionType> permissions =
        accountPermissions.stream().map(UserGroupController::mapAccountPermissions).collect(Collectors.toSet());
    return AccountPermissions.builder().permissions(permissions).build();
  }
  ...
  private QLAppFilter createAppFilterOutput(GenericEntityFilter appFilter) {
    if (isEmpty(appFilter.getIds())) {
      return QLAppFilter.builder().filterType(QLPermissionsFilterType.ALL).build();
    }
  ...
  public static List<QLAppPermissions> populateUserGroupAppPermissionOutput(Set<AppPermission> appPermissions) {
    List<QLAppPermissions> userGroupAppPermissions = null;
    if (appPermissions != null) {
      userGroupAppPermissions =
          appPermissions.stream().map(UserGroupController::convertToAppPermissionOutput).collect(Collectors.toList());
    }
    return userGroupAppPermissions;
  }
  ...
  private QLAppPermissions convertToAppPermissionOutput(AppPermission permission) {
    ...
    Set<QLActions> actions =
        actionsList.stream().map(UserGroupController::mapAppActionsToOutput).collect(Collectors.toSet());
    ...
    QLAppFilter appFilter = createAppFilterOutput(permission.getAppFilter());
  ...
  public static Set<QLAccountPermissionType> populateUserGroupAccountPermission(AccountPermissions permissions) {
    ...
    if (accountPermissions != null) {
      outputPermissions = accountPermissions.stream()
                              .map(UserGroupController::mapAccountPermissionsToOutput)
