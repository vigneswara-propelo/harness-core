  ...
  public void shouldReturnSshSUArtifactArgType() {
    ...
    Host host2 =
        aHost()
            ...
            .build();
    PageResponse<ServiceInstance> res = new PageResponse<>();
    res.setResponse(Lists.newArrayList(aServiceInstance().withUuid(getUuid()).withHost(host1).build(),
        aServiceInstance().withUuid(getUuid()).withHost(host2).build()));
    when(serviceInstanceService.list(anyObject())).thenReturn(res);
    when(hostService.list(any(PageRequest.class)))
        .thenReturn(aPageResponse().withResponse(Arrays.asList(host1, host2)).build());
    ...
    Host host2 =
        aHost()
            .withHostConnAttr(
                aSettingAttribute()
                    .withValue(
                        aHostConnectionAttributes().withAccessType(AccessType.USER_PASSWORD_SUDO_APP_USER).build())
                    .build())
            .build();
    PageResponse<ServiceInstance> res = new PageResponse<>();
    res.setResponse(Lists.newArrayList(aServiceInstance().withUuid(getUuid()).withHost(host1).build(),
