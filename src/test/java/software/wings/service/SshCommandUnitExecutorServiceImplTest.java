...
public class SshCommandUnitExecutorServiceImplTest extends WingsBaseTest {
  ...
  private static final SettingAttribute HOST_CONN_ATTR_KEY =
      aSettingAttribute().withValue(aHostConnectionAttributes().withAccessType(KEY).withKey(SSH_KEY).build()).build();
  ...
  public void shouldCreateBastionHostBasedSshConfig() {
    ...
    when(sshExecutorFactory.getExecutor(any(SshSessionConfig.class))).thenReturn(sshJumpboxExecutor);
    sshCommandUnitExecutorService.execute(host, EXEC_COMMAND_UNIT);
    verify(sshExecutorFactory).getExecutor(expectedSshConfig);
  }
