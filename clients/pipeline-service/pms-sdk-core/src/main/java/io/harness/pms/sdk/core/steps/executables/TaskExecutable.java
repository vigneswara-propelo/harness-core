 ...
 */
@OwnedBy(PIPELINE)
public interface TaskExecutable<T extends StepParameters, R extends ResponseData> extends Step<T> {
  ...
  default void handleAbort(
      Ambiance ambiance, T stepParameters, TaskExecutableResponse executableResponse, boolean userMarked) {
  ...
  default void handleFailure(
      Ambiance ambiance, T stepParameters, TaskExecutableResponse response, Map<String, String> metadata) {
  ...
  default ProgressData handleProgressTask(Ambiance ambiance, T stepParameters, ProgressData progressData) {
    // NOOP : By default this is noop as task expire is handled by the PMS, but you are free to override it
    return progressData;
  }
}
